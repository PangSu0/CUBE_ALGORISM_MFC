...
CUBE::CUBE(WCube* ppcb, rotation pFormula[]) 
{
	...
	formula = pFormula;
	pcb = ppcb;
	sd_position[sd_top] = color(pcb->getPiece(0, 1, 0)->getSidecolor(SD_TOP));
	sd_position[sd_left] = color(pcb->getPiece(-1, 0, 0)->getSidecolor(SD_LEFT));
	sd_position[sd_front] = color(pcb->getPiece(0, 0, 1)->getSidecolor(SD_FRONT));
	sd_position[sd_right] = color(pcb->getPiece(1, 0, 0)->getSidecolor(SD_RIGHT));
	sd_position[sd_back] = color(pcb->getPiece(0, 0, -1)->getSidecolor(SD_BACK));
	sd_position[sd_bottom] = color(pcb->getPiece(0, -1, 0)->getSidecolor(SD_BOTTOM)); 
	//면의 요소들 연결
	for (int y = 0; y < 3; y++)
		for (int x = 0; x < 2; x++)
		{
...
void CUBE::Run()
{
	...
	Chapter6();
	Chapter7();
	formula[formulaIndex++] = EndRotation;
}
...
void CUBE::CP1_ChangeColor(int _color)
{
	...
	CP1_SearchColor(pieptr, colorConect[(_color + 1) % 4], colorPosition[_color], SearColor[_color][0]);	//흰색 초록색 left 방향 탐색
